...
ifneq ($(filter $(UNAME_P),x86_64 i386),)
	cd qrack/build; $(CMAKE_L) -DENABLE_OPENCL=OFF -DFPPOW=6 -DENABLE_RDRAND=OFF -DENABLE_DEVRAND=ON -DQBCAPPOW=10 ..; make qrack_pinvoke qrack_cl_precompile
else
	cd qrack/build; $(CMAKE_L) -DENABLE_OPENCL=OFF -DFPPOW=6 -DENABLE_RDRAND=OFF -DENABLE_DEVRAND=ON -DENABLE_COMPLEX_X2=OFF -DENABLE_SSE3=OFF -DQBCAPPOW=10 ..; make qrack_pinvoke qrack_cl_precompile
endif
endif
ifeq ($(UNAME_S),Darwin)
ifneq ($(filter $(UNAME_P),x86_64 i386),)
