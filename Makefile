...
ifeq ($(UNAME_S),Linux)
ifeq ($(UNAME_P),x86_64)
	cd qrack/build; /usr/bin/cmake -DENABLE_RDRAND=OFF -DENABLE_DEVRAND=ON -DQBCAPPOW=7 ..; make qrack_pinvoke qrack_cl_precompile
else
	cd qrack/build; /usr/bin/cmake -DENABLE_RDRAND=OFF -DENABLE_DEVRAND=ON -DENABLE_COMPLEX_X2=OFF -DENABLE_SSE3=OFF -DQBCAPPOW=7 ..; make qrack_pinvoke qrack_cl_precompile
endif
endif
ifeq ($(UNAME_S),Darwin)
ifeq ($(UNAME_P),x86_64)
	cd qrack/build; cmake -DQBCAPPOW=6 ..; make qrack_pinvoke qrack_cl_precompile
else
